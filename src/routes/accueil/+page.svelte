                        ...
                        {#if !state.isLoadingChecklists && state.checklistsCount > 0}
                            <div class="mt-1" transition:fade>
                                <span class="bg-white/20 text-text-inverse px-3 py-1.5 rounded-full text-[10px] font-bold border border-white/10 backdrop-blur-sm inline-flex items-center">
                                    {state.checklistsCount} {state.checklistsCount > 1 ? 'checklists' : 'checklist'} en cours
                                </span>
                            </div>
                        {/if}
                    </div>
    ...
    <main class="flex-1 p-6 pb-28">
        {#if state.user}
            <div in:fade={{ duration: 300 }}>
                <div class="mt-4">
                    <h2 class="text-lg font-bold text-text-main mb-4 uppercase tracking-wider text-sm opacity-60">Checklists en cours</h2>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input 
                            type="text" 
                            id="checklistName"
                            bind:value={state.checklistName}
                    ...
                    <div>
                        ...
                        >
                            ...
                            {#each state.availableModels as model}
                    ...
                    <div class="flex flex-col gap-3 pt-4">
                        <button 
                            onclick={state.createChecklist}
                            disabled={!state.checklistName || !state.selectedModel || !!state.nameError || state.isCreating}
