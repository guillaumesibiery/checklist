...
/**
 * Vérifie si un item est considéré comme coché (quantité ajoutée >= quantité voulue)
 */
...
/**
 * Trie les items : non cochés en premier (ordre alphabétique), puis cochés (ordre alphabétique)
 */
function sortItemsByCheckedState(items: ChecklistItem[]): void {
    items.sort((a, b) => {
        const aChecked = isItemChecked(a);
        const bChecked = isItemChecked(b);
        // Les items non cochés viennent en premier
        if (aChecked !== bChecked) return aChecked ? 1 : -1;
        // Au sein d'un même groupe, tri alphabétique
...
export function createPageState(id: string, readOnly: boolean = false) {
    ...
    onMount(async () => {
        ...
        if (c) {
            // Trier les items : non cochés (alpha) puis cochés (alpha)
            c.elements.forEach(element => {
                sortItemsByCheckedState(element.items);
            });
            checklist = c;
    ...
    return {
        ...
        /** Catégories non complétées (avec leur index original dans elements) */
        get incompleteCategories(): { element: typeof checklist extends null ? never : NonNullable<typeof checklist>['elements'][number]; originalIndex: number }[] {
            if (!checklist) return [];
